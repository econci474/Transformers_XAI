"""
06_diagnostic_coverage_table.py
================================
Diagnostic coverage audit per ADNI visit code, showing:
  - Two modality columns: Clinical, MRI
  - Each with sub-columns: N | train | val | test
  - Rows grouped by visit (bl, m03, m06, ...)
  - Within each visit, sub-rows per seed (0, 1, 2) showing
    CN=X | MCI=Y | AD=Z counts
...
Outputs
-------
  outputs/diagnostic_coverage/diagnostic_coverage_table.csv
  outputs/diagnostic_coverage/diagnostic_coverage_table.png
  outputs/diagnostic_coverage/diagnostic_coverage_table.pdf

Run
---
  python clinical_pipeline/06_diagnostic_coverage_table.py
"""
...
#   Clinical N, MRI N
#   For each seed × split: count of CN, MCI, AD
#   Use Label_visit_diag for per-visit diagnosis

result_rows = []
for ses in sessions:
    # Clinical subjects at this visit
    clin_at_ses = clin[clin["VISCODE_long"] == ses].copy()
    clin_pids   = set(clin_at_ses["Patient_ID"].unique())
    clin_N      = len(clin_pids)

    # MRI subjects at this visit
    mri_at_ses = matched_only[matched_only["bids_ses"] == ses]
    mri_pids   = set(mri_at_ses["Patient_ID"].dropna().unique())
    mri_N      = len(mri_pids)

    # Build a per-subject diagnosis lookup at this visit
    # Use visit diagnosis (Label_visit_diag), deduplicated per patient
    clin_dx = (clin_at_ses
...
df_result.to_csv(OUT_DIR / "diagnostic_coverage_table.csv", index=False)
print(f"\nSaved CSV → {OUT_DIR / 'diagnostic_coverage_table.csv'}")
print(df_result.head(9).to_string(index=False))

# ── Trim sessions: keep only those with MRI > 0 (plus always sc, bl) ──────
sessions_with_mri = [s for s in sessions
                     if df_result[(df_result["Session"] == s)]["MRI_N"].max() > 0
                     or s in ("sc", "bl")]
# Further trim: last session with MRI > 0
last_mri_idx = max(
...
# ── Render the table ──────────────────────────────────────────────────────
# Layout:
#   Header row 1: | Session | Clinical                    | MRI                         |
#   Header row 2: |         | N | train | val | test      | N | train | val | test      |
#   Data:         | bl      | 616                         | 28                          |
#                 |  Seed 0 | CN=X | MCI=Y | AD=Z         | CN=X | MCI=Y | AD=Z        |
#                 |  Seed 1 | ...                          | ...                         |
#                 |  Seed 2 | ...                          | ...                         |
...
# row_type: "visit_header" | "seed_row"
...
# Column layout:
#   Col 0: Session (narrow)
#   Col 1: Clinical N
#   Col 2: Clinical train
#   Col 3: Clinical val
#   Col 4: Clinical test
#   Col 5: MRI N
#   Col 6: MRI train
...
TITLE_H    = 0.70
SUBTITLE_H = 0.50
HROW1_H    = 0.32  # "Clinical" / "MRI" header
HROW2_H    = 0.28  # N/train/val/test sub-header
TOP_PAD    = 0.12
BOT_PAD    = 0.12
FOOTNOTE_H = 0.80
...
ax.text((LEFT + RIGHT) / 2, (y_title_top + y_cursor) / 2,
        "ADNI Diagnostic Coverage by Visit and Seed Split\n"
...
hline(y_title_top, lw=1.5)  # top border
hline(y_cursor, lw=0.8)     # below subtitle

# ── Header row 1: Session | Clinical (spanning 4) | MRI (spanning 4) ─────
y_h1_top = y_cursor
y_cursor -= HROW1_H
...
for j, sh in enumerate(sub_headers):
    # Clinical sub-headers (cols 1-4)
    ax.text(col_cx[1 + j], y_h2_mid, sh,
            ha="center", va="center", fontsize=9, fontstyle="italic", color="black")
    # MRI sub-headers (cols 5-8)
    ax.text(col_cx[5 + j], y_h2_mid, sh,
            ha="center", va="center", fontsize=9, fontstyle="italic", color="black")

hline(y_cursor, lw=1.2)  # below header (midrule)

# ── Vertical separators ──────────────────────────────────────────────────
# Dotted vertical after Session column
xv_ses = col_left[1]
ax.plot([xv_ses, xv_ses], [y_cursor, y_cursor],  # placeholder, extend later
        color="black", linewidth=0.6, linestyle=(0, (3, 3)), zorder=2)

# Solid vertical between Clinical and MRI blocks
xv_mid = col_left[5]

# ── Data rows ─────────────────────────────────────────────────────────────
y_data_top = y_cursor  # remember for vertical lines
...
for item in display_data:
    if item[0] == "visit_header":
        ...
        # MRI N with percentage of Clinical N
        pct = (mri_N / clin_N * 100) if clin_N > 0 else 0
        ax.text(col_cx[5], y_mid, f"{mri_N} ({pct:.1f}%)",
                ha="center", va="center", fontsize=9, color="black")

        # Dotted horizontal line above each visit (except the first)
...
# ── Vertical lines ────────────────────────────────────────────────────────
# Dotted vertical after Session column (full data area)
ax.plot([xv_ses, xv_ses], [BOTTOM, y_data_top],
        color="black", linewidth=0.6, linestyle=(0, (3, 3)), zorder=2)

# Solid vertical between Clinical and MRI blocks (from header row 1 down)
...
footnote_raw = (
    ...
    f"Subjects not in any split (e.g. screening-only subjects) "
...
fig.savefig(OUT_DIR / "diagnostic_coverage_table.png", bbox_inches="tight", dpi=300)
fig.savefig(OUT_DIR / "diagnostic_coverage_table.pdf", bbox_inches="tight", dpi=300)
print(f"\nSaved PNG → {OUT_DIR / 'diagnostic_coverage_table.png'}")
print(f"Saved PDF → {OUT_DIR / 'diagnostic_coverage_table.pdf'}")
plt.close()
print("Done.")
