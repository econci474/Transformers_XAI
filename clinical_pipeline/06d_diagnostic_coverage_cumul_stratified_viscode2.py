"""
06d_diagnostic_coverage_cumul_stratified_viscode2.py
====================================================
VISCODE2-aligned version of 06d_diagnostic_coverage_cumul_stratified.py.

Cumulative diagnostic-coverage windows (yearly: ≤bl, ≤m12, ≤m24, …)
computed over the VISCODE_2 session keys, using the
viscode_2_aligned/master_mri_clinical_matched_viscode2.csv MRI master.

Outputs (same dir, same house style):
  outputs/diagnostic_coverage/diagnostic_coverage_table_cumul_stratified_viscode2.csv
  outputs/diagnostic_coverage/diagnostic_coverage_table_cumul_stratified_viscode2.png
  outputs/diagnostic_coverage/diagnostic_coverage_table_cumul_stratified_viscode2.pdf

Run
---
  python clinical_pipeline/06d_diagnostic_coverage_cumul_stratified_viscode2.py
"""
...
# ── Add month-number column (from VISCODE_2 / adni_viscode) ──────────────────
clin["_months"] = clin["VISCODE_2"].apply(session_to_months)
matched_only = matched_only.copy()
matched_only["_months"] = matched_only["adni_viscode"].apply(session_to_months)
...
# ── Pre-build diagnosis lookup (per-visit, VISCODE_2-keyed) ──────────────────
clin_dx_lookup = {}
for _, r in clin[["Patient_ID", "VISCODE_2", "Label_visit_diag"]].dropna(
        subset=["Label_visit_diag"]).iterrows():
    clin_dx_lookup[(r["Patient_ID"], r["VISCODE_2"])] = r["Label_visit_diag"]
print(f"  diagnosis lookup built: {len(clin_dx_lookup)} entries")

matched_only["_dx"] = matched_only.apply(
    lambda r: clin_dx_lookup.get((r["Patient_ID"], r["adni_viscode"])), axis=1)

# ── Per-window cumulative observation counts ─────────────────────────────────
result_rows = []
for cutoff in cutoffs:
    clin_cumul = clin[clin["_months"] <= cutoff]
    clin_N = len(clin_cumul)
...
df_result.to_csv(OUT_DIR / f"diagnostic_coverage_table_cumul_stratified_viscode2{SUFFIX}.csv", index=False)
print(f"\nSaved CSV → {OUT_DIR / ('diagnostic_coverage_table_cumul_stratified_viscode2' + SUFFIX + '.csv')}")
...
n_data_cols = 9
COL_W = [1.10, 0.70, 2.20, 2.20, 2.20, 1.30, 2.20, 2.20, 2.20]
...
ax.text((LEFT + RIGHT) / 2, (y_title_top + y_cursor) / 2,
        "ADNI Cumulative Diagnostic Coverage — STRATIFIED Splits  (VISCODE2-aligned)\n"
...
ax.text((LEFT + RIGHT) / 2, (y_sub_top + y_cursor) / 2,
        f"SNP+MRI cohort, n={n_subjects} subjects. Cumulative on VISCODE_2 months.\n"
...
fig.savefig(OUT_DIR / f"diagnostic_coverage_table_cumul_stratified_viscode2{SUFFIX}.png", bbox_inches="tight", dpi=300)
fig.savefig(OUT_DIR / f"diagnostic_coverage_table_cumul_stratified_viscode2{SUFFIX}.pdf", bbox_inches="tight", dpi=300)
print(f"\nSaved PNG → {OUT_DIR / ('diagnostic_coverage_table_cumul_stratified_viscode2' + SUFFIX + '.png')}")
print(f"Saved PDF → {OUT_DIR / ('diagnostic_coverage_table_cumul_stratified_viscode2' + SUFFIX + '.pdf')}")
plt.close()
print("Done.")
