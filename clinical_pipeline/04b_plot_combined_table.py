"""
04b_plot_combined_table.py
==========================
Renders a publication-quality table combining:
  1. Tabular baselines (LogReg, SVM, XGBoost)   from baseline CSV
  2. Encoder-only models (ModernBERT variants)   from metrics.json files

Same style as 02b_plot_baseline_table.py:
  - No shading/fills, all text black
  - Thick horizontal rules, dotted vertical separators
  - Bold group names, italic sub-headers, bold model labels

Outputs:
  outputs/combined_model_table.pdf
  outputs/combined_model_table.png
  outputs/combined_model_table.tex

Usage:
  python 04b_plot_combined_table.py
"""
...
OUT_DIR   = BASE / "outputs" / "encoder-only"
BL_CSV    = BASE / "outputs" / "baseline" / "baseline_model_comparison.csv"
ENC_DIR   = BASE / "outputs" / "encoder-only"
...
# Section 2: Encoder-only (frozen)
# Section 3: Encoder-only (full fine-tune)
ENC_MODELS = [
...
# Build row list: [(label, section, get_cell_fn), ...]
ROW_ENTRIES = []

def bl_cell(model_id, grp):
    """Retrieve a tabular baseline cell value."""
...
def enc_cell(model_id, strategy, grp):
    """Retrieve an encoder cell value (already aggregated)."""
...
def parse_mean(s):
    """Extract the mean value from a cell string like '0.9894 ± 0.0092'."""
    s = str(s).strip()
    if s in ("", "—", "nan", "NaN"):
        return float("-inf")
    try:
...
# Build cell_grid[row_idx] = list of cell strings (flat across all groups)
cell_grid = []
...
for col_idx in range(n_total_cols):
    ...
    best_val = max(means)
    if best_val > float("-inf"):  # at least one valid value
...
COL_W       = 1.45
ROW_H       = 0.32
ROW_H_SUB   = 0.22   # subtitle row (smaller)
MODEL_COL_W = 1.45
STRAT_COL_W = 0.90

n_data_cols = sum(len(g["headers"]) for g in GROUPS)
n_cols      = 2 + n_data_cols   # model + strategy + data

fig_w = 0.3 + MODEL_COL_W + STRAT_COL_W + n_data_cols * COL_W + 0.3
# title + group + subtitle + sub-headers + data rows
...
# ── Row geometry ───────────────────────────────────────────────────────────────
# Rows: 0=title, 1=group headers, 2=subtitle, 3=sub-headers, 4..=data
TOP = fig_h - 0.15
y_cur = TOP
row_tops = []
row_tops.append(y_cur); y_cur -= ROW_H      # 0: title
row_tops.append(y_cur); y_cur -= ROW_H      # 1: group headers
row_tops.append(y_cur); y_cur -= ROW_H_SUB  # 2: subtitle
row_tops.append(y_cur); y_cur -= ROW_H      # 3: sub-headers
for _ in range(n_rows):
    row_tops.append(y_cur); y_cur -= ROW_H  # 4+: data rows
row_tops.append(y_cur)                      # bottom edge

header_rows = 4  # title, group, subtitle, sub-header
...
hline(row_tops[3], lw=0.5, ls=(0, (3, 3)))             # below subtitle
hline(row_tops[4], lw=1.2)                             # below sub-headers
hline(row_tops[4 + len(BL_MODELS)], lw=0.6, ls="--")   # after tabular section
hline(row_tops[4 + len(BL_MODELS) + len(ENC_MODELS)], lw=0.6, ls="--")  # after frozen
hline(row_tops[total_visual_rows], lw=1.5)             # bottom border

# ── Dotted vertical lines between column groups ────────────────────────────────
col_cursor = 2  # start after model + strategy
for grp in GROUPS:
...
# Also a dotted line between model and strategy columns
x_strat = col_left[1]
...
ax.text((LEFT + RIGHT) / 2, row_mid(0),
        "Clinical Models: Test Performance at Baseline   "
...
# ── Subtitle row (row 2) — class distribution ─────────────────────────────────
col_cursor = 2
for grp in GROUPS:
    n = len(grp["headers"])
    x0 = col_left[col_cursor]
...
for r_idx, (strategy, label, model_id) in enumerate(ROW_ENTRIES):
    ...
    # Data cells (from pre-computed grid, bold if best)
    col_cursor = 2
...
lines = [
    r"\begin{table}[ht]",
    r"\centering",
    r"\caption{Clinical Models: Test Performance at Baseline (mean $\pm$ std, 3 seeds $\times$ 80/10/10 split)}",
    r"\label{tab:combined_models}",
