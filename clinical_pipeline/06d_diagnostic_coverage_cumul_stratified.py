"""
06d_diagnostic_coverage_cumul_stratified.py
============================================
Cumulative diagnostic coverage table using STRATIFIED baseline splits from
01c_build_clinical_csv_stratified.py (baseline_stratified/).

Identical rendering logic to 06b_diagnostic_coverage_table_cumul.py, but
reads from baseline_stratified/ and annotates the output accordingly.

Outputs
-------
  outputs/diagnostic_coverage/diagnostic_coverage_table_cumul_stratified.csv
  outputs/diagnostic_coverage/diagnostic_coverage_table_cumul_stratified.png
  outputs/diagnostic_coverage/diagnostic_coverage_table_cumul_stratified.pdf

Run
---
  python clinical_pipeline/06d_diagnostic_coverage_cumul_stratified.py
"""
...
df_result.to_csv(OUT_DIR / "diagnostic_coverage_table_cumul_stratified.csv", index=False)
print(f"\nSaved CSV → {OUT_DIR / 'diagnostic_coverage_table_cumul_stratified.csv'}")
...
ax.text((LEFT + RIGHT) / 2, (y_title_top + y_cursor) / 2,
        "ADNI Cumulative Diagnostic Coverage — STRATIFIED Splits\n"
...
fig.savefig(OUT_DIR / "diagnostic_coverage_table_cumul_stratified.png", bbox_inches="tight", dpi=300)
fig.savefig(OUT_DIR / "diagnostic_coverage_table_cumul_stratified.pdf", bbox_inches="tight", dpi=300)
print(f"\nSaved PNG → {OUT_DIR / 'diagnostic_coverage_table_cumul_stratified.png'}")
print(f"Saved PDF → {OUT_DIR / 'diagnostic_coverage_table_cumul_stratified.pdf'}")
plt.close()
print("Done.")
