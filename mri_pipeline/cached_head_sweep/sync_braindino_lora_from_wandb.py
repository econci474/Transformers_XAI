"""
sync_braindino_lora_from_wandb.py
=================================
Synthesise per-run `metrics.json` shims for the BrainDINO LoRA sweep,
from W&B project `braindino_lora`. The trainer
(`mri_pipeline/brain_dino/02_supervised_finetuning_BrainDINO.py`) logs the
full set of val_* metrics to W&B (val_bacc, val_auc, val_f1, val_sens/spec,
val_TP/FP/TN/FN) but NOT test/*; those only land in the canonical
metrics.json on CSD3.

So shims contain full val coverage; test cells stay blank until the user
rsyncs the canonical metrics.json down from CSD3.

Output layout matches the aggregator's BrainDINO LoRA glob:

    braindino_outputs/lora/aug_<aug>/BrainDINO_vitb16_lora/<task>/seed_<n>/metrics.json

Usage:
    python mri_pipeline/cached_head_sweep/sync_braindino_lora_from_wandb.py
    python mri_pipeline/cached_head_sweep/sync_braindino_lora_from_wandb.py --augment plus_original
"""
...
WANDB_CSV = REPO_ROOT / "mri_pipeline" / "cached_head_sweep" / "wandb_braindino_lora.csv"
DERIV_ROOT = Path(r"D:/ADNI_BIDS_project/derivatives/braindino_outputs/lora")
...
def parse_args():
    ...
    p.add_argument("--augment", default=None,
                   help="Restrict to one augment (none|random|stochastic|"
                        "plus_original). Default: all.")
...
def _row_to_metrics(row: pd.Series) -> dict:
    """Map a flattened W&B run row into the BrainDINO trainer's metrics.json
    schema. Only val_* fields are present in W&B summary; test_metrics stays
    empty so the user knows to rsync the canonical file later."""
    ...
    return {
        "config": {
            ...
            "lora_rank":             int(row["config/lora_rank"]),
        ...
        "val_metrics": {
            # NOTE: balanced_acc here is the BEST seen across training (the
            # checkpoint-selection metric). The W&B `summary/val_bacc` field
            # is the LAST epoch's val_bacc by W&B's default-summary behaviour,
            # so we deliberately copy summary/best_val_balanced_acc into this
            # slot instead -- matches what the cross-model table reads from
            # config.best_val_balanced_acc.
        ...
        # Empty -- BrainDINO trainer computes test metrics but only writes them
        # to disk (metrics.json on CSD3), not to W&B. Rsync the canonical
        # metrics.json later to populate the test cells.
        "test_metrics": {},
...
def main():
    ...
    if not WANDB_CSV.exists():
        raise SystemExit(f"W&B CSV not found: {WANDB_CSV}. Run "
                         "06_fetch_wandb_tables.py --projects braindino_lora first.")
    ...
    # If multiple runs share the same (task, seed, augment) (e.g. resubmits),
    # keep the one with the highest best_val_balanced_acc.
    ...
    for _, row in df.iterrows():
        ...
        task = row["config/task"]
        seed = int(row["config/seed"])
        out_dir = DERIV_ROOT / f"aug_{aug}" / "BrainDINO_vitb16_lora" / task / f"seed_{seed}"
