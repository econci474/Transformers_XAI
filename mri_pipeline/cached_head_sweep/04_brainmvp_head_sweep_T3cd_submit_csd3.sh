...
#SBATCH --job-name=bmvp_head_T3cd
#SBATCH --output=/home/ec474/rds/hpc-work/ADNI_MRI/brainmvp_debug/slurm_logs/bmvp_head_T3cd_%A_%a.log
#SBATCH --error=/home/ec474/rds/hpc-work/ADNI_MRI/brainmvp_debug/slurm_logs/bmvp_head_T3cd_%A_%a.err
#SBATCH -p icelake
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
# =============================================================================
# 04_brainmvp_head_sweep_T3cd_submit_csd3.sh
# =============================================================================
# Cached-embedding head HP sweep on the NEW T3c (7y) and T3d (10y) prognostic
# tasks. Extends the existing T3a/T3b family with longer horizons. 2 tasks x
# 3 seeds x 3 LRs x 3 drops x 2 LSs = 108 cells. ~30 min wall-clock on
# icelake at %4 throttle.
#
# Caveat: Label_7y / Label_10y in the master CSV use a simple "any AD dx
# within N years" rule (01b_build_clinical_csv.py::compute_prognosis); they
# do NOT mask subjects with FU < N years, so a subject with 3 y of FU who
# never had an AD dx is labelled 0 at Label_10y -- conservative-biased. For
# T3d (10y) this affects most ADNI3 subjects. A censored-aware Label_Ny
# would be a separate downstream improvement; results from this sweep are
# still useful as a baseline.
#
# T3c/T3d are registered in TASK_CONFIG (mri_pipeline/04_supervised_finetuning_ViT.py).
#
# Submit:  sbatch mri_pipeline/cached_head_sweep/04_brainmvp_head_sweep_T3cd_submit_csd3.sh
# =============================================================================
...
MODEL_NAME="BrainMVP"
EMBED_DIM=512
WANDB_PROJECT="${WANDB_PROJECT:-brainmvp_frozen_cached_T3cd}"
export WANDB_MODE="${WANDB_MODE:-offline}"

TASKS=("T3c_conv7y" "T3d_conv10y")
SEEDS=(0 1 2)
...
echo "============================================================"
echo "  ${MODEL_NAME} cached-head sweep -- T3c/T3d"
