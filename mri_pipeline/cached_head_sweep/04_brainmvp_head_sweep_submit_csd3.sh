...
# node: `cd mri_pipeline/cached_head_sweep && wandb sweep sweep_brainmvp.yaml`
# -> returns a sweep_id. Pass it via:
#   sbatch --export=ALL,SWEEP_ID=<id> 04_brainmvp_head_sweep_submit_csd3.sh
# If unset, cells run as individual W&B runs grouped under `head_sweep_BrainMVP`.
SWEEP_ID="${SWEEP_ID:-}"
SWEEP_FLAG=""
if [ -n "$SWEEP_ID" ]; then
