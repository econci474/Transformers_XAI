"""
sync_braindino_frozen_aug_from_wandb.py
=======================================
Synthesise per-run `metrics.json` shims for the BrainDINO frozen-encoder
on-the-fly-aug sweep (W&B project `braindino_frozen_aug`).

Same trainer as the LoRA sweep
(`mri_pipeline/brain_dino/02_supervised_finetuning_BrainDINO.py`); the
trainer logs the full set of val_* metrics to W&B but does NOT log test/*
-- those only land in the canonical metrics.json on CSD3.

So shims fill val cells; test cells stay blank until you rsync/scp the
canonical metrics.json down from CSD3 (under
`/home/ec474/rds/hpc-work/ADNI_MRI/braindino_outputs/frozen/`).

Output layout matches the aggregator's BrainDINO frozen-aug glob:

    braindino_outputs/frozen/aug_<aug>/BrainDINO_vitb16_frozen/<task>/seed_<n>/metrics.json

Usage:
    python mri_pipeline/cached_head_sweep/sync_braindino_frozen_aug_from_wandb.py
    python mri_pipeline/cached_head_sweep/sync_braindino_frozen_aug_from_wandb.py --augment plus_original
"""
...
WANDB_CSV = REPO_ROOT / "mri_pipeline" / "cached_head_sweep" / "wandb_braindino_frozen_aug.csv"
DERIV_ROOT = Path(r"D:/ADNI_BIDS_project/derivatives/braindino_outputs/frozen")
...
def parse_args():
    ...
    p.add_argument("--augment", default=None,
                   help="Restrict to one augment (stochastic|plus_original). "
                        "Default: all (the sweep doesn't cover 'none' -- that's "
                        "the cached-head BrainDINO/frozen/none row).")
...
def _row_to_metrics(row: pd.Series) -> dict:
    ...
    schema. Only val_* fields are present in W&B summary."""
    ...
    return {
        ...
        "val_metrics": {
            # balanced_acc here is the BEST seen across training (checkpoint-
            # selection metric); summary/val_bacc is the last epoch, so we
            # deliberately copy summary/best_val_balanced_acc instead.
        ...
        # Empty -- BrainDINO trainer writes test metrics only to disk
        # (metrics.json on CSD3), not to W&B. Rsync to fill the test cells.
        "test_metrics": {},
...
def main():
    ...
    if not WANDB_CSV.exists():
        raise SystemExit(f"W&B CSV not found: {WANDB_CSV}. Run "
                         "06_fetch_wandb_tables.py --projects braindino_frozen_aug first.")
    ...
    # Dedup retries: keep the highest-val_bacc per (task, seed, augment).
    ...
    for _, row in df.iterrows():
        ...
        task = row["config/task"]
        seed = int(row["config/seed"])
        out_dir = DERIV_ROOT / f"aug_{aug}" / "BrainDINO_vitb16_frozen" / task / f"seed_{seed}"
