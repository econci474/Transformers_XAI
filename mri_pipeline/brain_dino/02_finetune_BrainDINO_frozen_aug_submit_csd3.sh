...
#SBATCH --job-name=bdino_frozen_aug
#SBATCH --output=/home/ec474/rds/hpc-work/ADNI_MRI/braindino_outputs/frozen/slurm_logs/bdino_frozen_aug_%A_%a.log
#SBATCH --error=/home/ec474/rds/hpc-work/ADNI_MRI/braindino_outputs/frozen/slurm_logs/bdino_frozen_aug_%A_%a.err
#SBATCH -p ampere
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
...
#SBATCH --array=0-29%4
# =============================================================================
# 02_finetune_BrainDINO_frozen_aug_submit_csd3.sh  --  frozen + live aug sweep
# =============================================================================
# Fine-tunes BrainDINO ViT-B/16 with the encoder frozen, training only the
# classification head -- BUT with live input augmentation (stochastic /
# plus_original) that requires re-running the encoder forward per epoch.
#
# Why a separate submit (not the existing frozen submit):
#   The aug=none cells were already done via the cached-embedding sweep at
#   braindino_outputs/aug_none/BrainDINO_vitb16_frozen_cached/ (deterministic
#   encoder forward, embeddings cached to .pt). This script covers the OTHER
#   two augments where augmentation has to happen on the volume each epoch,
#   so cached embeddings would discard the augmentation signal.
#
# Hyperparams reconciled against runners/classification.py's frozen config:
# lr=1e-4, weight_decay=1e-5, optimizer=Adam, scheduler=ReduceLROnPlateau
# (mode=max, factor=0.5, patience=10), label_smoothing=0.0.
#
# 5 tasks x 3 seeds x 2 augments = 30 combinations (array 0-29).
# Array decoder: AUG_IDX -> SEED_IDX -> TASK_IDX (low-to-high stride).
#
#   Task    : T1_binary | T1b_binary | T1c_binary | T1d_binary | T2_multiclass
#   Seed    : 0 | 1 | 2
#   Augment : stochastic | plus_original   (aug=none done via cached sweep)
#
# Output layout: braindino_outputs/frozen/aug_${AUGMENT}/BrainDINO_vitb16_frozen/<task>/seed_<n>/
# (nested under frozen/ for parallel to lora/ + ft/ from today's refactor;
#  the legacy aug_none/BrainDINO_vitb16_frozen_cached/ tree is untouched.)
#
# Pre-flight (run once):
#   mkdir -p /home/ec474/rds/hpc-work/ADNI_MRI/braindino_outputs/frozen/slurm_logs
# =============================================================================
...
# Strategy-scoped subroot: parallel to lora/ + ft/.
STRATEGY_OUT_DIR="${OUT_DIR}/frozen"

PY_SESSION_FLAGS="--long all"
STRATEGY="frozen"
WANDB_PROJECT="${WANDB_PROJECT:-braindino_frozen_aug}"
export WANDB_MODE="${WANDB_MODE:-offline}"

# -- Combination lookup: 5 tasks x 3 seeds x 2 augments = 30 ------------------
TASKS=("T1_binary" "T1b_binary" "T1c_binary" "T1d_binary" "T2_multiclass")
SEEDS=(0 1 2)
AUGMENTS=("stochastic" "plus_original")
...
echo "============================================================"
echo "  BrainDINO ViT-B/16 frozen + live aug"
