...
#SBATCH --job-name=vit_ft_bl
#SBATCH --output=logs/vit_ft_bl_%A_%a.log
#SBATCH --error=logs/vit_ft_bl_%A_%a.err
#SBATCH -p ampere
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
...
#SBATCH --array=0-11
# =============================================================================
# 04c_finetune_ViT_bl_submit_csd3.sh
# =============================================================================
# Runs T1/T2 only on baseline scans (--session bl) across 3 seeds x 2
# strategies. Companion to:
...
#
# Why this exists
#   The master 04 sweep used --long 1, which gave T1/T2 a bl + m12 cohort
#   (~526 scans). To support an apples-to-apples comparison with the
#   bl-only T3a/T3b runs (and to motivate the bl+m12 inclusion in the
#   thesis table), we need T1/T2 results on the bl-only cohort (N~28).
#
...
#   sbatch mri_pipeline/04c_finetune_ViT_bl_submit_csd3.sh             # full 12-job sweep
#   sbatch --array=0  mri_pipeline/04c_finetune_ViT_bl_submit_csd3.sh  # smoke (T1/seed0/full_ft)
# =============================================================================
...
# Sister output dir to keep bl-only and bl+m12 results separate
OUT_DIR="/home/ec474/rds/hpc-work/ADNI_MRI/vit_outputs_bl"

# -- Session selection: ses-bl only ------------------------------------------
PY_SESSION_FLAGS="--session bl"

# -- Combination lookup: T1 / T2 only ----------------------------------------
...
echo "============================================================"
echo "  ViT supervised fine-tuning -- bl-only sweep (T1/T2)"
...
echo "  Sessions       : ${PY_SESSION_FLAGS}  (ses-bl only, ~28 scans)"
